...
void process(uint8_t* images, int num_images, int img_width, int img_height,
    int32_t* serialized_pairs, int32_t* labels, int num_examples, 
    const char* db_filename) {

    // Temporary storage for assembling image pairs
    const size_t img_size = 3 * img_width * img_height;
    uint8_t* serialized_pair_buf = new uint8_t[img_size];
    ...
    for (int i = 0; i < num_examples; ++i) {
        ...
        memcpy(serialized_pair_buf, img, img_size);

        datum.set_data(serialized_pair_buf, img_size);
    ...
    out:
    delete [] serialized_pair_buf;
}

void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
    if (nrhs != 4) {
        mexErrMsgTxt("Must have 4 inputs"); 
    }
    if (nlhs != 0) {
    ...
    // Read serialized pairs
    if (!mxIsInt32(prhs[1]) || mxGetN(prhs[1]) != 1) {
        mexErrMsgTxt("pairs must have type int32 and have 1 column");
    }
    int32_t* serialized_pairs = (int32_t*) mxGetData(prhs[1]);

    // Read labels
    if (!mxIsInt32(prhs[2]) || mxGetN(prhs[2]) != 1) {
        mexErrMsgTxt("labels must be an int32 column vector");
    }
    int32_t* labels = (int32_t*) mxGetData(prhs[2]);

    int num_examples = mxGetM(prhs[1]);
    if (num_examples != mxGetM(prhs[2])) {
        mexErrMsgTxt("Inconsistent number of examples");
    }

    // Read db filename
    char* db_filename = mxArrayToString(prhs[3]);

    google::InitGoogleLogging("pairs_to_caffedb");
    process(images, num_images, width, height, serialized_pairs, labels, num_examples, db_filename);
