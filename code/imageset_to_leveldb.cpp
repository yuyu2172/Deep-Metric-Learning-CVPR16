...
void process(uint8_t* images, int num_images, int img_width, int img_height, const char* db_filename) {
    ...
    for (int i=0; i<num_images; i++) {
        uint8_t* img = &images[i * img_size];

        memcpy(buf, img, img_size);

        datum.set_data(buf, img_size);
        datum.set_label(0);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
    if (nrhs != 2) {
        mexErrMsgTxt("Must have 2 inputs"); 
    }
    if (nlhs != 0) {
    ...
    // Read db filename
    char* db_filename = mxArrayToString(prhs[1]);

    google::InitGoogleLogging("pairs_to_caffedb");
    process(images, num_images, width, height, db_filename);
